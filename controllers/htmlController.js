								...
								.then(function (distinctTaskFromBidOpenRes) {
									...
									allUserBiddedTasksClosed(req, res)
										.then(function (distinctTaskFromBidClosedRes) {
											...
											//     raw.push(allUserTaskOpen[i].get({ plain: true }))
											// }
											const rawOpenData = allUserTaskOpenRes.map(seqObj => seqObj.get({
												plain: true
											}))
											const rawClosedData = allUserTaskClosedRes.map(seqObj => seqObj.get({
												plain: true
											}))
											const rawDistinctOpenData = distinctTaskFromBidOpenRes.map(seqObj => seqObj.get({
												plain: true
											}))
											const rawDistinctClosedData = distinctTaskFromBidClosedRes.map(seqObj => seqObj.get({
												plain: true
											}))
